...
def init_db():
    ...
    try:
        execute_query('ALTER TABLE products ADD COLUMN subcategory_id INTEGER', commit=True)
    except: pass
    try:
...
# --- دوال المنتجات ---
def _canonical_seed_categories(rows):
    names = []
    for row in rows:
        name = (row.get('seed_category_name') or '').strip()
...
def _get_or_create_category(name, sort=999):
    ...
    if existing:
        execute_query('UPDATE categories SET visible=1, sort=? WHERE id=?', (sort, existing['id']), commit=True)
        return existing['id']

    return execute_query(
        'INSERT INTO categories (name, icon, visible, sort) VALUES (?, ?, 1, ?)',
        (name, '', sort),
        commit=True
    )
...
def _hide_noncanonical_seed_taxonomy(category_names, subcategory_ids):
    if category_names:
        ...
        execute_query(
            f'UPDATE categories SET visible=0 WHERE name NOT IN ({placeholders})',
...
def seed_products_from_csv():
    ...
    for sort_index, row in enumerate(rows, start=1):
        try:
            ...
            price_raw = (row.get('final_price') or row.get('karaz_price') or '').replace('\u20aa', '').replace('?', '').strip()
...
def categorize_seed_products():
    rows = _seed_rows()
    if rows is None:
        return {'updated': 0, 'errors': 0, 'missing_file': True}
    ...
    for row in rows:
        try:
            name = (row.get('final_name') or row.get('karaz_name') or '').strip()
            barcode = (row.get('final_barcode') or '').strip()
            ...
            if barcode:
                ...
                existing = execute_query('SELECT id FROM products WHERE TRIM(name)=TRIM(?)', (name,), fetchone=True)
...
def calculate_delivery_fee(lat, lng):
    ...
    return round(max(10, min(25, delivery)), 1)
